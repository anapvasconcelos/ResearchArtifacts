...
tex_files <- list.files("processed-data", pattern="\\.tex$", full.names = TRUE)
...
for (tex in tex_files) {
  ...
  pdf_path <- file.path("processed-data", pdf_file)
  ...
  if (file.exists(bib_file)) {
    file.rename(bib_file, file.path("outputs", bib_file))
  }
