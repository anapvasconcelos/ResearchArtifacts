...
events <- read_excel("processed-data/dataset.xlsx", sheet = "FSETimelineData")
...
year_breaks <- seq(min(timeline_data$Year), max(timeline_data$Year), by = 1)
min_year <- min(year_breaks)
max_year <- max(year_breaks)
...
p <- ggplot(timeline_data, aes(x = Year, y = Event)) +
  ...
  geom_rect(
    data = bg_data,
    mapping = aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = fill),
    inherit.aes = FALSE,
    alpha = 0.15,
    color = NA
  ) +
  ...
  geom_hline(yintercept = seq_along(levels(timeline_data$Event)) - 0.5,
  ...
  theme(
    ...
    plot.margin = margin(t = -10, r = 4, b = -10, l = 5)
  ) +
  labs(x = "", y = "FSE Event")
...
#ggsave("outputs/F02-FSE-timeline.jpg", plot = p, width = 6.7, height = 10, dpi = 300)
ggsave("outputs/F02-FSE-timeline.pdf", plot = p, width = 6.7, height = 10)
